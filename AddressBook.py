...
class AddressBook(UserList):
    ...
    def __next__(self):
        phones = ', '.join(filter(None, self.data[self.counter]['phones']))
        birth = self.data[self.counter]['birthday'].strftime("%d/%m/%Y") if self.data[self.counter]['birthday'] else ''
        result = "_" * 50 + "\n" \
                 f"Name: {self.data[self.counter]['name']} \n" \
                 f"Phones: {phones} \n" \
                 f"Birthday: {birth} \n" \
                 f"Email: {self.data[self.counter]['email']} \n" \
                 f"Status: {self.data[self.counter]['status']} \n" \
                 f"Note: {self.data[self.counter]['note']}\n" \
                 "_" * 50
        self.counter += 1
            ...
            for account in self.data:
                ...
                if account['name'] == contact_name:
                    if parameter == 'birthday':
                        ...
                        new_value = [Phone(number).value for number in new_contact]
        ...
        for account in self.data:
            if account['birthday']:
                ...
                if self.__get_current_week()[0] <= new_birthday.date() < self.__get_current_week()[1]:
                    ...
                    result.append(f"{weekday}: {account['name']}")
        return "_" * 50 + '\n' + '\n'.join(result) + '\n' + "_" * 50